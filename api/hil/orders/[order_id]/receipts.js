...
module.exports = async function handler(req, res) {
  ...
  if (!serviceUrl) return send(res, 503, { error: 'order_service_unavailable', order_id: orderId, receipts: [] });

  const headers = { accept: 'application/json' };
  if (process.env.HIL_ORDER_SERVICE_TOKEN) headers.authorization = `Bearer ${process.env.HIL_ORDER_SERVICE_TOKEN}`;
  try {
    const upstream = await fetch(`${serviceUrl.replace(/\/$/, '')}/orders/${encodeURIComponent(orderId)}/receipts`, {
      method: 'GET', headers, signal: AbortSignal.timeout(10000),
    });
    const payload = await upstream.json().catch(() => ({}));
    return send(res, upstream.status, payload);
  } catch (error) {
    return send(res, 503, { error: 'order_service_request_failed', order_id: orderId, receipts: [], detail: String(error && error.message || error) });
  }
};
