...
The mathematics educator is broader than a single equation solver. It should support explanation, hints, guided solutions, checking work, alternate methods, prerequisites, notation, proof structure, history/foundations, philosophy where relevant, and separately admitted solver/tool execution.
...
Uncertain transcription must never silently become mathematical fact.
...
The original Math-Solver Governance Adapter was published as a `RESEARCH_NOTE`. Its source-to-instruction, governed packet, artifact-return, admissibility-result, and replay concepts remain useful for formalism/deep-work execution, but they now sit behind the shared conversational capability rather than defining a separate public application.
...
Completion requires a real mathematics request through `ecosystem-chat.html`, correct specialty routing, governed solver/tool execution where applicable, result binding, and deterministic replay/verification evidence required by Site#239/#240. No duplicate provider/runtime authority may be created to satisfy this goal.
