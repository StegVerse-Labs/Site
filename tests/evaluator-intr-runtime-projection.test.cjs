...
async function boot(value){
  ...
  const context={
    ...
    fetch:async(url,options)=>{
      ...
      if(url==="data/evaluator-review/runtime-projection.json"){
...
(async()=>{
  ...
  const stale=await boot(projection({
    public_route_observed_at:"2026-01-01T00:00:00Z",
    gateway_readiness_observed_at:"2026-01-01T00:00:00Z",
    public_route_max_age_seconds:60
  }));
