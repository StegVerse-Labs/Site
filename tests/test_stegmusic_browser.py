...
def snapshot(page,stage):
    raw=page.locator('#rawEvents').text_content() or ''
    return {'stage':stage,'audio_notice':page.locator('#audioNotice').text_content() or '','play_button':page.locator('#playPause').text_content() or '','progress':float(page.locator('#progress').input_value()),'composition_phase':page.locator('#compositionPhase').text_content() or '','track_title':page.locator('#trackTitle').text_content() or '','event_types':event_types(page),'raw_events_excerpt':raw[-4000:],'runtime_loaded':page.evaluate("typeof window.StegMusicRuntime === 'object'"),'diagnostics_loaded':page.evaluate("typeof window.StegMusicDiagnostics === 'object'"),'transition_scheduler_loaded':page.evaluate("typeof window.StegMusicTransitionScheduler === 'object'")}
def main():
    ...
    try:
        with sync_playwright() as p:
            ...
            before_title=playing['track_title']; page.locator('#adaptiveNext').click(); page.wait_for_timeout(2800); adaptive=snapshot(page,'after_adaptive_next'); result['stages'].append(adaptive)
            ...
            checks={'page_loaded':page.locator('#playPause').count()==1,'base_runtime_loaded':playing['runtime_loaded'],'diagnostic_runtime_loaded':playing['diagnostics_loaded'],'transition_scheduler_loaded':playing['transition_scheduler_loaded'],'audio_context_running_marker':'running locally' in playing['audio_notice'].lower(),'composition_advanced':playing['progress']>0,'playback_started_event':'playback_started' in observed,'adaptive_decision_event':'adaptive_selection_decision' in observed,'transition_scheduled_event':'stegdj_transition_scheduled' in observed,'transition_executed_event':'stegdj_transition_executed' in observed,'track_changed_after_bounded_transition':adaptive['track_title']!=before_title,'playback_paused_event':'playback_paused' in observed,'pause_returned_control':paused['play_button'].strip().lower()=='play','no_page_errors':not result['page_errors']}
