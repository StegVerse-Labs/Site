...
def main():
    ...
    try:
        with sync_playwright() as p:
            ...
            page.locator('[data-six-track="3"]').click(); page.locator('#playPause').click()
            render_completed=True
            try:
                page.wait_for_function("document.querySelector('#rawEvents')?.textContent.includes('\\\"event_type\\\":\\\"enhanced_media_playback_started\\\"')",timeout=30000)
            ...
            checks={'page_loaded':page.locator('#playPause').count()==1,'six_track_registry_loaded':registry,'runtime_track_count_is_six':count==6,'all_six_titles_selectable':result['selected_titles']==EXPECTED,'enhancement_loaded':page.evaluate("typeof window.StegMusicEnhancement==='object'"),'render_completed_within_timeout':render_completed,'enhanced_audio_active':'normalized' in notice and 'mix playing' in notice,'composition_advanced':progress>0,'registry_ready_event':'stegmusic_six_track_registry_ready' in observed,'enhanced_playback_event':'enhanced_media_playback_started' in observed,'no_page_errors':not result['page_errors']}
