...
var DEVICE_LOCAL_ENDPOINT = "https://stegverse.org/stegos-bootstrap/local-model";
...
var prohibited = ["token", "access_token", "github_token", "gh_token", "authorization", "provider_credential", "provider_credentials", "secret", "private_key"];
Object.keys(value).forEach(function (key) {
if (prohibited.indexOf(String(key).toLowerCase()) !== -1) {
...
function permittedScope(scope) { return scope === "loopback" || scope === "private" || scope === "stegverse-local"; }
...
if (raw.execution_authority !== false || raw.authority_effect !== "NONE") { throw new Error("FAIL_CLOSED: TVC route authority escalation"); }
...
endpoint_transport: raw.endpoint_transport || "PRIVATE_HTTP",
...
if (bundle.schema !== "stegos.web_canonical_inference_evidence.v1") { throw new Error("FAIL_CLOSED: canonical inference evidence schema mismatch"); }
...
if (route.endpoint_transport === "SERVICE_WORKER_LOCAL_INTERCEPT" && route.endpoint !== DEVICE_LOCAL_ENDPOINT) {
throw new Error("FAIL_CLOSED: service-worker route endpoint mismatch");
}
return {
...
}).then(function (entry) { db.close(); return { evidence: admitted, entry: entry }; });
});
});
});
}
function bootstrapDeviceLocalInferenceEvidence() {
if (!(navigator.serviceWorker && navigator.serviceWorker.ready)) {
return Promise.reject(new Error("FAIL_CLOSED: service worker unavailable for device-local inference"));
}
return navigator.serviceWorker.ready.then(function () {
return fetch("./local-model/canonical-evidence", { credentials: "omit", cache: "no-store" });
}).then(function (response) {
if (!response.ok) { throw new Error("FAIL_CLOSED: device-local evidence endpoint returned HTTP " + response.status); }
if (response.headers.get("X-StegVerse-Execution") !== "SERVICE_WORKER_LOCAL_INTERCEPT") {
throw new Error("FAIL_CLOSED: device-local evidence was not produced by service-worker interception");
}
return response.json();
}).then(importCanonicalInferenceEvidence);
}
...
return fetch(evidence.endpoint.replace(/\/$/, "") + "/v1/chat/completions", {
method: "POST",
...
if (evidence.endpoint_transport === "SERVICE_WORKER_LOCAL_INTERCEPT" && response.headers.get("X-StegVerse-Execution") !== "SERVICE_WORKER_LOCAL_INTERCEPT") {
throw new Error("FAIL_CLOSED: admitted service-worker route escaped device-local execution");
}
return response.json();
}).then(function (result) {
rejectProtectedMaterial(result, "inference_result");
var usage = result.usage_proof || result.usage;
if (usage && usage.measured !== true && result.usage && typeof result.usage.total_tokens === "number") {
usage = {
measured: true,
model_used: true,
...
if (!usage || usage.measured !== true || usage.model_used !== true) { throw new Error("FAIL_CLOSED: model usage proof missing"); }
var responseModel = result.model_id || result.model;
if (responseModel && responseModel !== LOCAL_MODEL_ID) { throw new Error("FAIL_CLOSED: inference response model identity mismatch"); }
...
endpoint_transport: evidence.endpoint_transport,
...
function reflectDeviceLocalAdmission(result) {
var state = document.getElementById("evidence-admission-state");
var inferenceState = document.getElementById("admitted-inference-state");
var routeState = document.getElementById("inference-state");
var output = document.getElementById("evidence-admission-output");
var button = document.getElementById("run-inference");
if (state) { state.textContent = "ADMITTED_DEVICE_LOCAL"; }
if (inferenceState) { inferenceState.textContent = "READY"; }
if (routeState) { routeState.textContent = "ADMITTED_STEGVERSE_DEVICE_LOCAL_ROUTE_READY"; }
if (button) { button.disabled = false; }
if (output && result && result.evidence) { output.textContent = JSON.stringify(result.evidence, null, 2); }
}
function installDeviceLocalAdmissionUI() {
var text = document.getElementById("canonical-evidence");
var button = document.getElementById("admit-evidence");
if (text) { text.placeholder = "Leave empty to admit the canonical on-device StegVerse model/TVC route, or paste an external canonical evidence bundle."; }
if (button) {
...
var state = document.getElementById("evidence-admission-state");
var output = document.getElementById("evidence-admission-output");
if (state) { state.textContent = "CHECKING_DEVICE_LOCAL"; }
bootstrapDeviceLocalInferenceEvidence().then(reflectDeviceLocalAdmission).catch(function (error) {
if (state) { state.textContent = "FAIL_CLOSED"; }
if (output) { output.textContent = String(error && error.message ? error.message : error); }
});
}, false);
}
readAdmittedInferenceEvidence().then(function (evidence) {
if (evidence && evidence.state === "ADMITTED") { return null; }
return bootstrapDeviceLocalInferenceEvidence().then(reflectDeviceLocalAdmission);
}).catch(function () { /* Node may not yet be established; explicit UI retry remains available. */ });
}
