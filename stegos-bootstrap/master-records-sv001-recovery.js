...
(function (root) {
  ...
  function without(object, key) {
    var copy = {};
    Object.keys(object || {}).forEach(function (name) { if (name !== key) { copy[name] = object[name]; } });
    return copy;
  }

  function normalizeJournal(input) {
    var rows = Array.isArray(input) ? input : (input && Array.isArray(input.continued_receipts) ? input.continued_receipts : null);
    if (!rows || !rows.length) { fail("retained journal entries required"); }
    rows = rows.slice().sort(function (a, b) { return a.sequence - b.sequence; });
    return rows;
  }
  ...
  function verifyJournal(rows) {
    ...
    rows.forEach(function (entry, index) {
      chain = chain.then(function () {
        ...
        }).then(function (entryDigest) {
          if (entryDigest !== entry.entry_sha256) { fail("journal entry hash mismatch"); }
        });
      });
    });
  ...
  function byEntryHash(rows, hash) {
    return rows.find(function (entry) { return entry.entry_sha256 === hash; }) || null;
  }

  function bySequence(rows, sequence) {
    return rows.find(function (entry) { return entry.sequence === sequence; }) || null;
  }
  ...
  function validateCheckout(entry, binding) {
    ...
    if (receipt.task_id !== TASK_ID || receipt.claim_id !== binding.claim_id || receipt.fencing_token !== binding.fencing_token) {
      fail("WorkerCoordinator claim/fence lineage mismatch");
    }
    ...
    if (receipt.execution_surface !== "CURRENT_USER_IPHONE" || receipt.credential_authority !== "TV/TVC" || receipt.github_token_runtime_authority !== "NONE") {
      fail("WorkerCoordinator execution/credential boundary mismatch");
    }
    if (receipt.receipt_sha256 !== binding.admission_receipt_sha256) { fail("binding does not reference checkout receipt"); }
    return shaPrefixed(without(receipt, "receipt_sha256")).then(function (digest) {
      if (digest !== receipt.receipt_sha256) { fail("WorkerCoordinator receipt self-hash mismatch"); }
      return receipt;
    });
  }
  ...
  function validateIssuance(entry, binding) {
    ...
    var receipt = entry.receipt;
    if (receipt.transition_id !== "TVC_SV001_BOUNDED_AUTONOMY_LEASE_ISSUED" || receipt.workercoordinator_claim_id !== binding.claim_id ||
        receipt.workercoordinator_fencing_token !== binding.fencing_token || receipt.lease_id !== binding.lease_id || receipt.lease_hash !== binding.lease_hash) {
      fail("TVC issuance lineage mismatch");
    }
    if (receipt.single_cycle !== true || receipt.github_actions_runtime_authority !== "NONE") { fail("TVC issuance authority boundary mismatch"); }
    return shaPrefixed(without(receipt, "receipt_hash")).then(function (digest) {
      if (digest !== receipt.receipt_hash) { fail("TVC issuance receipt self-hash mismatch"); }
      return receipt;
    });
  }
  ...
  function buildCandidate(binding, preExecutionEntries, preExecutionTail, completedAt) {
    return {
      ...
      lease_sha256: binding.lease_hash,
      candidate_task: {
      ...
      observations: {
        ...
        stegos_node_id: binding.node_id,
        pre_execution_journal_entries: preExecutionEntries,
        pre_execution_journal_tail_sha256: preExecutionTail
      },
      authorized_execution_source: AUTHORIZED_SOURCE,
  ...
  async function recover(input, expectedReceiptHash) {
    ...
    var rows = normalizeJournal(input);
    await verifyJournal(rows);

    var terminals = rows.filter(function (entry) {
      var receipt = entry.receipt || {};
      return receipt.schema === TERMINAL_SCHEMA && receipt.task_id === TASK_ID && receipt.profile_id === PROFILE &&
        receipt.state === "COMPLETED" && receipt.transition_id === TRANSITION && receipt.cycle_receipt_hash === expectedReceiptHash;
    });
    if (terminals.length !== 1) { fail("exactly one canonical terminal receipt required"); }

    var terminalEntry = terminals[0];
    var terminal = terminalEntry.receipt;
    var bindingEntry = byEntryHash(rows, terminal.binding_entry_sha256);
    ...
    if (binding.task_id !== TASK_ID || binding.profile_id !== PROFILE || binding.claim_id !== terminal.claim_id ||
        binding.fencing_token !== terminal.fencing_token || binding.credential_authority !== "TV/TVC" ||
        binding.global_workercoordinator_authority !== false || binding.external_claim_promoted_to_browser_authority !== false || binding.carrier_granted_authority !== false) {
      fail("external binding lineage/authority mismatch");
    }
    ...
    var issuanceEntry = byEntryHash(rows, bindingEntry.previous_entry_sha256);
    if (!issuanceEntry || issuanceEntry.sequence !== bindingEntry.sequence - 1) { fail("TVC issuance adjacency missing"); }
    var checkoutEntry = byEntryHash(rows, issuanceEntry.previous_entry_sha256);
    if (!checkoutEntry || checkoutEntry.sequence !== issuanceEntry.sequence - 1) { fail("WorkerCoordinator checkout adjacency missing"); }
    await validateCheckout(checkoutEntry, binding);
    await validateIssuance(issuanceEntry, binding);

    var reconstructionEntry = bySequence(rows, terminalEntry.sequence + 1);
    if (!reconstructionEntry || !reconstructionEntry.receipt || reconstructionEntry.receipt.schema !== RECONSTRUCTION_SCHEMA ||
        reconstructionEntry.receipt.terminal_entry_sha256 !== terminalEntry.entry_sha256 || reconstructionEntry.receipt.state !== "PASS" ||
        reconstructionEntry.receipt.same_execution !== true) { fail("same-execution reconstruction receipt missing"); }

    var consumptionEntry = bySequence(rows, reconstructionEntry.sequence + 1);
    if (!consumptionEntry || !consumptionEntry.receipt || consumptionEntry.receipt.schema !== CONSUMPTION_SCHEMA ||
        consumptionEntry.receipt.lease_id !== binding.lease_id || consumptionEntry.receipt.lease_hash !== binding.lease_hash ||
        consumptionEntry.receipt.execution_receipt_sha256 !== expectedReceiptHash || consumptionEntry.receipt.lease_consumption_state !== "CONSUMED" ||
        consumptionEntry.receipt.credential_authority !== "TV/TVC" || consumptionEntry.receipt.github_token_runtime_authority !== "NONE" ||
        consumptionEntry.receipt.heartbeat_granted_authority !== false) { fail("TVC lease-consumption evidence missing or inconsistent"); }

    var start = Date.parse(binding.bound_at);
    var end = Date.parse(terminal.completed_at);
    ...
    var matches = [];
    for (var timestamp = start; timestamp <= end; timestamp += 1) {
      var candidate = buildCandidate(binding, bindingEntry.sequence - 1, bindingEntry.previous_entry_sha256, new Date(timestamp).toISOString());
      var digest = await shaPrefixed(candidate);
      if (digest === expectedReceiptHash) { matches.push(candidate); }
    }
    if (matches.length !== 1) { fail("canonical cycle receipt is not uniquely recoverable from retained journal"); }

    var source = matches[0];
    source.receipt_hash = expectedReceiptHash;
    return {
      schema: "stegverse.master-records.sv001-canonical-journal-recovery/v1",
      state: "RECOVERED_HASH_VERIFIED",
      source_receipt: source,
      source_receipt_sha256: expectedReceiptHash,
      source_journal_window: {
        checkout_sequence: checkoutEntry.sequence,
        tvc_issuance_sequence: issuanceEntry.sequence,
        binding_sequence: bindingEntry.sequence,
        terminal_sequence: terminalEntry.sequence,
        reconstruction_sequence: reconstructionEntry.sequence,
        consumption_sequence: consumptionEntry.sequence
      },
      matched_completed_at: source.completed_at,
  ...
  root.StegVerseMasterRecordsSv001CanonicalJournalRecovery = {
    recover: recover,
