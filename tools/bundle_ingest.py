...
def stale_receipt(path: Path, reason: str, seen_state: str | None = None) -> dict[str, Any]:
    return {
        ...
        "formal_milestone": "MS-012E.2 — Already-Seen Incoming Quarantine Fix",
        "incoming_path": str(path),
        "file_name": path.name,
        "file_sha256": sha_f(path),
...
def already_seen(root: Path, bundle_sha: str, policy: dict[str, Any]) -> str | None:
    return seen_status(root, bundle_sha, policy)
...
def ingest_one(root: Path, bundle: Path, policy_path: Path, apply: bool, retry_failed: bool = False) -> dict[str, Any]:
    ...
    seen = None if retry_failed else already_seen(root, bundle_sha, policy)
    if seen:
    ...
    if gate["verdict"] == "PRIVILEGED_EXECUTOR_REQUIRED":
        ...
        if apply:
            copy_receipt(root, bundle, policy.get("queue", {}).get("privileged_dir", "privileged_queue"), receipt, "privileged-task")
    ...
    if gate["verdict"] in {"SANDBOX_REQUIRED", "HUMAN_REVIEW_REQUIRED", "FAIL_CLOSED"}:
        ...
        if apply:
            copy_receipt(root, bundle, route, receipt, "failure")
    ...
    for path in entries:
        ...
        try:
            ...
            reports.append({
                ...
                "mode": "apply" if apply else "dry_run",
                "receipt": receipt,
                "summary": {"stale_quarantined": 1 if apply else 0},
            })
            ...
            if apply:
                copy_receipt(root, path, policy.get("queue", {}).get("failed_dir", "failed_bundles"), receipt, "failure")
            reports.append({
                "generated_at": receipt["generated_at"],
                "schema": "stegverse.bundle_ingestion_report.v1",
                "bundle": str(path),
...
def write_queue_report(report: dict[str, Any], out_dir: Path) -> None:
    ...
    for item in report.get("reports", []):
        ...
        lines.append(
            f"- `{receipt.get('bundle_name') or receipt.get('file_name')}` → "
            f"`{receipt.get('verdict')}` / `{receipt.get('route')}` — {receipt.get('reason')}"
        )

    for receipt in report.get("stale_receipts", []):
        lines.append(
            f"- `{receipt.get('file_name')}` → "
            f"`{receipt.get('verdict')}` / `{receipt.get('route')}` — {receipt.get('reason')}"
        )
