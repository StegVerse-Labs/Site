...
def receipts_html(element_id, receipt_index):
    ...
    for r in receipts[-20:]:
        extras = ""
        for key, label in [("lag_flip", "Lag flip"), ("decision_flip", "Decision flip"), ("actuation_flip", "Actuation flip"), ("total_lag_flip", "Total lag flip"), ("trust_flip", "Trust flip"), ("coupling_flip", "Coupling flip"), ("local_admissible_coupled_denied", "Local admissible / coupled denied"), ("composition_flip", "Composition flip"), ("all_individual_allow", "All individual allow"), ("conflict_detected", "Conflict detected"), ("conflict_flip", "Conflict flip"), ("consensus_flip", "Consensus flip"), ("receipt_bound", "Receipt bound"), ("reconstruction_exact", "Reconstruction exact"), ("reconstruction_verdict_match", "Reconstruction verdict match"), ("irreversible", "Irreversible"), ("point_of_no_return", "Point of no return"), ("self_modification_safe", "Self-modification safe")]:
...
def row_constraint(row):
    ...
    if row.get("mode") in ("capacity_margin_sweep_v1", "observation_lag_sweep_v1", "decision_lag_sweep_v1", "actuation_lag_sweep_v1", "trust_drift_sweep_v1", "two_state_coupling_sweep_v1", "multi_agent_sweep_v1", "conflict_sweep_v1", "consensus_sweep_v1", "receipt_bound_sweep_v1", "reconstruction_sweep_v1", "irreversibility_sweep_v1", "self_modifying_rule_sweep_v1"):
        return row.get("constraint_result", "PASS")
    return "not applicable"
...
def automation_release_html(automation_releases):
    ...
    if not releases:
        return "<p>No automation release state has been generated yet.</p>"
    items = "".join(
        f"<li><strong>{esc(item.get('mode'))}</strong> · {esc(item.get('status'))} · released by {esc(', '.join(item.get('released_by', [])))}<br>{esc(item.get('description', ''))}</li>"
...
def receipt_backing_html(eid, receipt_backing):
    checks = [c for c in receipt_backing.get("checks", []) if c.get("element_id") == eid]
    promotions = [p for p in receipt_backing.get("promotions", []) if p.get("element_id") == eid]
    if not checks and not promotions:
        return "<p>No receipt-backed verification has been run for this transition yet.</p>"
    blocks = []
    for check in checks:
        failures = check.get("failures", [])
        failure_text = "none" if not failures else esc(failures)
        blocks.append(f"""
        <article class="change-entry">
          <h3>Receipt-backed verifier</h3>
          <p><strong>Verified:</strong> {esc(check.get("verified"))}</p>
          <p><strong>Checked rows:</strong> {esc(check.get("checked_rows"))}</p>
          <p><strong>Receipt count:</strong> {esc(check.get("receipt_count"))}</p>
          <p><strong>Failures:</strong> {failure_text}</p>
        </article>
        """)
    for promotion in promotions:
        blocks.append(f"""
        <article class="change-entry">
          <h3>Evidence promotion</h3>
          <p><strong>From:</strong> {esc(promotion.get("from"))}</p>
          <p><strong>To:</strong> {esc(promotion.get("to"))}</p>
          <p><strong>Reason:</strong> {esc(promotion.get("reason"))}</p>
        </article>
        """)
    return "\\n".join(blocks)
