(() => {
  ...
  const COPY = Object.freeze({
    en: {
      viewLabel: 'Ecosystem Node view', conversation: 'Conversation', governed: 'Governed record', split: 'Split',
      rawJsonl: 'Raw JSONL', formatted: 'Formatted records', exportJson: 'Export JSON', exportJsonl: 'Export JSONL',
      streamTitle: 'Canonical governed event stream', streamSummary: 'Formatted inspection and raw output resolve to the same in-memory records.',
      authority: 'projection only · authority unchanged', conversationProjection: 'Conversation projection', governedProjection: 'Governed record projection',
      eventId: 'event_id', parent: 'parent', actor: 'actor', hash: 'hash', locale: 'Language'
    },
    es: {
      viewLabel: 'Vista del Nodo del Ecosistema', conversation: 'Conversación', governed: 'Registro gobernado', split: 'Vista dividida',
      rawJsonl: 'JSONL sin formato', formatted: 'Registros formateados', exportJson: 'Exportar JSON', exportJsonl: 'Exportar JSONL',
      streamTitle: 'Flujo canónico de eventos gobernados', streamSummary: 'La inspección formateada y la salida sin formato resuelven a los mismos registros en memoria.',
      authority: 'solo proyección · autoridad sin cambios', conversationProjection: 'Proyección de conversación', governedProjection: 'Proyección del registro gobernado',
      eventId: 'event_id', parent: 'evento padre', actor: 'actor', hash: 'hash', locale: 'Idioma'
    },
    'zh-Hans': {
      viewLabel: '生态系统节点视图', conversation: '对话', governed: '治理记录', split: '分屏',
      rawJsonl: '原始 JSONL', formatted: '格式化记录', exportJson: '导出 JSON', exportJsonl: '导出 JSONL',
      streamTitle: '规范治理事件流', streamSummary: '格式化检查与原始输出解析到同一组内存记录。',
      authority: '仅为投影 · 权限不变', conversationProjection: '对话投影', governedProjection: '治理记录投影',
      eventId: 'event_id', parent: '父事件', actor: '参与者', hash: '哈希', locale: '语言'
    },
    'zh-Hant': {
      viewLabel: '生態系統節點檢視', conversation: '對話', governed: '治理記錄', split: '分割檢視',
      rawJsonl: '原始 JSONL', formatted: '格式化記錄', exportJson: '匯出 JSON', exportJsonl: '匯出 JSONL',
      streamTitle: '規範治理事件流', streamSummary: '格式化檢查與原始輸出解析到同一組記憶體記錄。',
      authority: '僅為投影 · 權限不變', conversationProjection: '對話投影', governedProjection: '治理記錄投影',
      eventId: 'event_id', parent: '父事件', actor: '參與者', hash: '雜湊', locale: '語言'
    }
  });
  const stream = [];
  ...
  controls.innerHTML = `
    ...
    <div class="node-view-actions">
      <label class="node-locale-control"><span data-locale-label>Language</span>
        <select id="nodeLocale" aria-label="Language">
  ...
  workspace.innerHTML = `
    ...
    <section class="node-pane governed-pane" aria-label="Governed record projection">
      <div class="governed-record-head">
        <div><strong data-stream-title>Canonical governed event stream</strong><span data-stream-summary>Formatted inspection and raw output resolve to the same in-memory records.</span></div>
        <span class="node-authority-badge" data-authority-badge>projection only · authority unchanged</span>
      </div>
  ...
  const localeSelect = document.getElementById('nodeLocale');
  ...
  function resolveInitialLocale() {
    const stored = localStorage.getItem('stegverse-node-locale');
    if (SUPPORTED_LOCALES.includes(stored)) return stored;
    const browserLocale = navigator.language || 'en';
    if (browserLocale.toLowerCase().startsWith('es')) return 'es';
    if (browserLocale.toLowerCase().startsWith('zh')) return browserLocale.toLowerCase().includes('hant') || /-(tw|hk|mo)$/i.test(browserLocale) ? 'zh-Hant' : 'zh-Hans';
    return 'en';
  }
  ...
  function setLocale(locale) {
    ...
    activeLocale = locale;
    localStorage.setItem('stegverse-node-locale', locale);
    applyLocale();
  ...
  function applyLocale() {
    ...
    controls.querySelector('.node-view-tabs').setAttribute('aria-label', copy.viewLabel);
    controls.querySelector('[data-node-view="conversation"]').textContent = copy.conversation;
    controls.querySelector('[data-node-view="governed"]').textContent = copy.governed;
    controls.querySelector('[data-node-view="split"]').textContent = copy.split;
    controls.querySelector('[data-locale-label]').textContent = copy.locale;
    localeSelect.setAttribute('aria-label', copy.locale);
    document.getElementById('nodeFormatToggle').textContent = rawMode ? copy.formatted : copy.rawJsonl;
    document.getElementById('nodeExportJson').textContent = copy.exportJson;
    document.getElementById('nodeExportJsonl').textContent = copy.exportJsonl;
    workspace.querySelector('.conversation-pane').setAttribute('aria-label', copy.conversationProjection);
    workspace.querySelector('.governed-pane').setAttribute('aria-label', copy.governedProjection);
    workspace.querySelector('[data-stream-title]').textContent = copy.streamTitle;
    workspace.querySelector('[data-stream-summary]').textContent = copy.streamSummary;
    workspace.querySelector('[data-authority-badge]').textContent = copy.authority;
  }
  ...
  function ingestMessageElement(element) {
    ...
    const event = createCanonicalEvent({
      ...
      event_type: 'message',
      human_projection: { label, body, visible_in_conversation: true },
      governed_projection: {
      ...
      evidence_refs: [], artifact_refs: [], continuity_refs: parentEvent ? [parentEvent.event_id] : []
    });
    ...
    if (!isUser) {
      appendCanonicalEvent(createCanonicalEvent({
        ...
        governed_projection: {
          decision: 'PREVIEW_ONLY', admissibility: 'NOT_EVALUATED', execution_requested: false,
          execution_result: 'NOT_ATTEMPTED', confidence: null,
          uncertainty: 'No live provider or authority-issued receipt is represented.'
        },
        policy_refs: ['policy:site-preview-boundary'], evidence_refs: [event.event_id], artifact_refs: [], continuity_refs: [event.event_id]
      }));
    }
  }
  ...
  function createCanonicalEvent(partial) {
    ...
    const event = {
      event_id: eventId, parent_event_id: partial.parent_event_id ?? null, timestamp: new Date().toISOString(),
      actor: partial.actor || {}, event_type: EVENT_TYPES.has(partial.event_type) ? partial.event_type : 'evidence',
      human_projection: partial.human_projection || {}, governed_projection: partial.governed_projection || {},
      policy_refs: partial.policy_refs || [], evidence_refs: partial.evidence_refs || [], artifact_refs: partial.artifact_refs || [],
      continuity_refs: partial.continuity_refs || [], hash: ''
    };
  ...
  function renderGovernedRecords() {
    ...
    stream.forEach((event) => {
      ...
      article.tabIndex = 0;
      article.innerHTML = `
        <div class="governed-event-title"><strong>${escapeHtml(event.event_type)}</strong><span>${escapeHtml(event.timestamp)}</span></div>
        <dl>
          <div><dt>${escapeHtml(copy.eventId)}</dt><dd>${escapeHtml(event.event_id)}</dd></div>
          <div><dt>${escapeHtml(copy.parent)}</dt><dd>${escapeHtml(event.parent_event_id || 'null')}</dd></div>
          <div><dt>${escapeHtml(copy.actor)}</dt><dd>${escapeHtml(event.actor.display_name || event.actor.actor_type || 'unknown')}</dd></div>
          <div><dt>${escapeHtml(copy.hash)}</dt><dd>${escapeHtml(event.hash)}</dd></div>
        </dl>
  ...
  function selectEvent(eventId, source) {
    ...
    if (source === 'governed') chatLog.querySelector(`[data-event-id="${cssEscape(eventId)}"]`)?.scrollIntoView({ block: 'nearest', behavior: 'smooth' });
    else recordList.querySelector(`[data-event-id="${cssEscape(eventId)}"]`)?.scrollIntoView({ block: 'nearest', behavior: 'smooth' });
  }
  ...
  function toggleRecordFormat() {
    ...
    document.getElementById('nodeFormatToggle').textContent = rawMode ? COPY[activeLocale].formatted : COPY[activeLocale].rawJsonl;
  }

  function exportRecords(format) {
    const content = format === 'jsonl' ? stream.map((event) => JSON.stringify(event)).join('\n') + '\n'
      : JSON.stringify({ schema: 'stegverse.canonical-event-stream.v0.1', locale: activeLocale, events: stream }, null, 2);
  ...
  window.StegVerseCanonicalEventStream = Object.freeze({
    version: '0.2', getEvents: () => stream.slice(), getEvent: (eventId) => eventIndex.get(eventId) || null,
    getLocale: () => activeLocale, setLocale, supportedLocales: SUPPORTED_LOCALES.slice(), selectEvent
  });
})();