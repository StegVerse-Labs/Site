(() => {
  ...
  function validateTargetNodeRef(value) {
    if (typeof value !== 'string' || !NODE_REF_RE.test(value)) throw new Error('FAIL_CLOSED: canonical sovereign resident target required');
    return value;
  }

  function validateSubmitterProvenanceRef(value) {
    if (typeof value !== 'string' || !PROVENANCE_RE.test(value)) throw new Error('FAIL_CLOSED: validated Node Receipt #1 provenance required');
    return value;
  }

  async function resolveSubmitterProvenance(nodeApi = globalThis.StegVerseNodeContinuity) {
    if (!nodeApi || typeof nodeApi.status !== 'function') throw new Error('FAIL_CLOSED: canonical StegVerse Node continuity unavailable');
    ...
    if (!genesis || genesis.receipt_number !== 1 || genesis.transition !== 'NODE_REGISTERED') throw new Error('FAIL_CLOSED: validated Node Receipt #1 unavailable');
    if (reg.node_id !== genesis.node_id || reg.receipt_sha256 !== genesis.receipt_sha256 || !RECEIPT_SHA_RE.test(String(reg.receipt_sha256 || ''))) {
  ...
  function validateDiscovery(payload) {
    if (!payload || payload.schema !== DISCOVERY_SCHEMA || payload.consumer !== CONSUMER ||
        payload.current_resident_request_id !== CURRENT_REQUEST_ID || payload.gateway_execution_authority !== 'NONE' ||
        payload.credential_authority !== 'TV/TVC' || payload.discovery_grants_authority !== false ||
    ...
    return Object.freeze({
      state:'AVAILABLE', target_node_ref:payload.target_node_ref, expires_at:payload.expires_at || null,
      current_resident_request_id:CURRENT_REQUEST_ID, gateway_execution_authority:'NONE',
      credential_authority:'TV/TVC', authority_effect:'NONE_DISCOVERY_ONLY'
    });
  }

  async function discover({ gatewayBaseUrl, fetchImpl = globalThis.fetch }) {
  ...
  async function submitDiscovered({
    ...
    const result = await submit({ gatewayBaseUrl, targetNodeRef: discovery.target_node_ref, authorizationRef, leaseMs, fetchImpl });
    return Object.freeze({
      ...result,
