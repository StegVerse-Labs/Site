        ...
        return intr.buildMaterializationRequest(
          ...
          return node.queueIntrMaterializationRequest(materialization).then(function(){
            return sync.synchronizeMaterialization(materialization.materialization_id).then(function(){
              ...
              var localResultEligible=(recordClass===DIRECTORY_CLASS||recordClass===HEALTH_CLASS);requireValue(deliveryReceipt&&((deliveryReceipt.network_delivery_observed===true)||(localResultEligible&&deliveryReceipt.local_ingress_observed===true)),"DEVICE_KV query ingress delivery not observed");
