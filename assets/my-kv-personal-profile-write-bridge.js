...
function transact(profile){
 ...
 return node.status().then(function(s){
  ...
  var nodeId=s.registration.node_id,q=buildRequest(nodeId,profile),payload=new TextEncoder().encode(canon(q));
  return intr.buildIntent("device-kv",payload,"COMMIT_CANDIDATE",q.request_id)
   ...
   .then(function(m){return node.queueIntrMaterializationRequest(m).then(()=>sync.synchronizeMaterialization(m.materialization_id)).then(()=>sync.loadTarget()).then(function(target){
    ...
    return poll(target,lookup,0).then(function(delivery){
      requireValue(delivery&&delivery.schema===RESULT_SCHEMA&&delivery.state==="RESULT_AVAILABLE","Personal KV write delivery invalid");
      var response=delivery.response;
      requireValue(response&&response.schema==="stegverse.device-kv.profile-update-response/v1"&&response.state==="PROFILE_PERSISTED","Personal KV write was not confirmed");
      requireValue(response.request_id===q.request_id&&response.canonical_path===DEST&&response.exact_readback_verified===true&&response.authority_effect==="NONE","Personal KV write binding invalid");
      return {persisted:true,state:"KV_PERSISTED",message:"Personal information saved to KnowledgeVault.",receipt_hash:delivery.receipt_hash||null,response:response};
    });
   });});
 });
}
root.StegVerseKVPersonalInfoBridge=Object.freeze({bridge_kind:"DEVICE_KV_PERSONAL_PROFILE_UPDATE",authority_effect:"NONE",saveProfile:transact,loadProfile:null});
