...
def main() -> int:
    ...
    if vacc.get("specialty_destination") != "va-claims-chat.html":
        fail("VACC specialty destination mismatch")

    math = by_id["mathematics_educator"]
    if math.get("image_transcription_must_be_correctable") is not True:
        fail("math image transcription correction boundary missing")
    if math.get("specialty_destination") != "math-solver/index.html":
        fail("mathematics specialty destination mismatch")

    hil = by_id["hil_experiment"]
    if hil.get("experiment_specific_surface_allowed") is not True:
        fail("HIL experiment surface exception missing")
    if hil.get("specialty_destination") != "humans-as-interoperability-layer.html":
        fail("HIL specialty destination mismatch")

    four_app = json.loads(FOUR_APP_STATUS.read_text(encoding="utf-8"))
    four_topology = four_app.get("topology_semantics") or {}
    if four_topology.get("legacy_four_app_name_is_accounting_only") is not True:
        fail("legacy four-app status not reconciled")
    if four_topology.get("competing_primary_chat_applications") is not False:
        fail("legacy four-app status still permits competing primary chats")
    if four_topology.get("primary_public_conversational_surface") != data["primary_surface"]:
        fail("legacy four-app status primary surface mismatch")

    two_entry = json.loads(TWO_ENTRY_STATUS.read_text(encoding="utf-8"))
    two_topology = two_entry.get("topology_semantics") or {}
    if two_topology.get("legacy_two_entry_name_is_historical_accounting_only") is not True:
        fail("legacy two-entry status not reconciled")
    if two_topology.get("vacc_is_specialty_capability") is not True:
        fail("legacy two-entry status does not classify VACC as specialty")
    if two_topology.get("va_claims_chat_is_alternate_primary_chat") is not False:
        fail("legacy two-entry status still permits VA Claims Chat as alternate primary chat")
    if two_topology.get("primary_public_conversational_surface") != data["primary_surface"]:
        fail("legacy two-entry status primary surface mismatch")
