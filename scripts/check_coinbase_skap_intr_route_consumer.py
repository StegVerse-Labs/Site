...
def main():
    ...
    for marker in ("config?.submission_status !== 'PROVISIONED'","submission_allowed_origins.length !== 1","submission_allowed_origins[0] !== endpoint.origin","public_route_hostname !== endpoint.hostname","public_route_observation_digest","public_route_observed_at","public_route_max_age_seconds","ready_for_owner_ingress !== true","provider_operation_authorized !== false","provider_operation_started !== false","submission_blind_retry_allowed !== false","credentials: 'omit'","redirect: 'error'","referrerPolicy: 'no-referrer'","VERIFY_EXTERNALLY","blind retry forbidden","response.next_required_transition !== 'KV_SKAP_VAULT_INTERLOCK_ADMISSION'"):
        require(marker in submit,'missing submission invariant '+marker)
    for prohibited in ("trycloudflare.com","EXPLICIT_FALLBACK","FALLBACK_CARRIER","validateFallbackRoute"):
        require(prohibited not in submit,'third-party fallback remains '+prohibited)
