...
def main():
    ...
    for marker in ("credential_custody_target!=='SKAP'","transport_protocol!=='InTr'","liveness_receipt_hash","recipient_runtime_instance_id:config.runtime_instance_id","recipient_lease_expires_at:config.lease_expires_at","'d' in config.recipient_public_jwk","sealCoinbaseCredential"):
        require(marker in ingress,'missing sealing invariant '+marker)
    for marker in ("route?.status !== 'ROUTE_LIVE'","route?.[field] !== config?.[field]","endpoint.hostname.endsWith('.trycloudflare.com')","credentials: 'omit'","redirect: 'error'","referrerPolicy: 'no-referrer'","VERIFY_EXTERNALLY","blind retry forbidden","decryption_performed_at_ingress !== false","rewrap_performed_at_ingress !== false","execution_authority !== 'NONE'","may_authorize_order !== false","sealed_material_persisted_unchanged !== true"):
