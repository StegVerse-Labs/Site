...
SOURCES = [
    {"task_id":"MC-01-CRYPTO-ACCESSIBILITY","repository":"StegVerse-Labs/crypto-bot","issue":"StegVerse-Labs/crypto-bot#7","workflow":".github/workflows/first-accessibility-mark.yml","evidence_path":"data/first-accessibility-mark-status.json","complete":lambda v:v.get("status")=="PASS" and v.get("paper_trading_accessible") is True,"stop_condition":"status=PASS and paper_trading_accessible=true","completion_action":"The canonical crypto-bot owner regenerates the committed first-accessibility receipt."},
    {"task_id":"MC-02-MARKETPLACE-COLLECTION","repository":"GCAT-BCAT-Engine/Marketplace","issue":"GCAT-BCAT-Engine/Marketplace#1","workflow":".github/workflows/import-marketplace-coinbase-settlements.yml","evidence_path":"data/marketplace-coinbase-outbound-collection-status.json","complete":lambda v:v.get("status")=="COLLECTED","stop_condition":"status=COLLECTED; acknowledgement ACCEPTED or DUPLICATE; sequence-2 transport present","completion_action":"The canonical Marketplace owner retains collection, acknowledgement, sequence-2 generation, and durable status."},
    {"task_id":"MC-03-PUBLISHER-VERIFY","repository":"GCAT-BCAT-Engine/Publisher","issue":"GCAT-BCAT-Engine/Publisher#19","workflow":".github/workflows/collect-marketplace-coinbase-release-evidence.yml","evidence_path":"data/marketplace-coinbase-release-evidence-status.json","complete":lambda v:v.get("status")=="VERIFIED" and v.get("paper_release_verified") is True,"stop_condition":"status=VERIFIED and paper_release_verified=true","completion_action":"The canonical Publisher owner retains bounded reconstruction, verification, and committed public status."},
    {"task_id":"MC-04-SITE-PROJECTION","repository":"StegVerse-Labs/Site","issue":"StegVerse-Labs/Site#131","workflow":".github/workflows/import-marketplace-coinbase-accessibility.yml","evidence_path":"data/marketplace-coinbase-accessibility-status.json","complete":lambda v:v.get("state")=="PAPER_ACCESSIBLE" and v.get("live_trading_accessible") is False,"stop_condition":"state=PAPER_ACCESSIBLE and live_trading_accessible=false","completion_action":"The canonical Site projection owner retains regeneration after Publisher verification."},
]
...
def fetch_json(repository: str, path: str) -> tuple[str, dict, str | None]:
    ...
    headers = {"Accept":"application/vnd.github+json","User-Agent":"StegVerse-Marketplace-Coinbase-Observation/3.0","X-GitHub-Api-Version":"2022-11-28"}
    try:
...
def build_payload() -> dict:
    ...
    for source in SOURCES:
        ...
        if complete:
            ...
            next_action = "The anonymous observation path cannot see this evidence. The named repository/issue remains the canonical owner; do not repair observation by introducing a GitHub token."
        else:
            state = "RETRY"
            next_action = source["completion_action"] + " StegVerse-owned observation may retry later."
        tasks.append({"task_id":source["task_id"],"repository":source["repository"],"issue":source["issue"],"workflow":source["workflow"],"evidence_path":source["evidence_path"],"observation":observation,"observation_detail":detail,"state":state,"stop_condition":source["stop_condition"],"observed_status":value.get("status", value.get("state")),"development_halt":False,"completion_action":source["completion_action"],"next_action":next_action})
    return {"schema":"stegverse.site.marketplace_coinbase_activation_tasks.v3","controller_issue":"StegVerse-Labs/Site#131","generated_at":datetime.now(timezone.utc).isoformat().replace("+00:00","Z"),"state":"COMPLETE" if all_complete else "ACTIVE_STEGVERSE_CONTINUATION","development_halt":False,"continuation_mode":"STEGVERSE_OWNED_OBSERVATION_ONLY","controller_access":{"credential_requirement":"NONE","github_token_allowed":False,"non_tv_tvc_secret_or_token_allowed":False,"anonymous_public_observation_only":True,"access_failure_is_not_task_failure":True,"access_failure_owner":"StegVerse-Labs/Site#131"},"tasks":tasks,"authority":{"publication":False,"release":False,"execution":False,"live":False,"financial":False}}
