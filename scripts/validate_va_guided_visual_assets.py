...
EXPECTED = [
    'card-1-get-ready.svg','card-2-choose-sign-in.svg','card-3-create-account.svg',
    'card-4-sign-in-va.svg','card-5-download-records.svg','card-6-preserve-continue.svg',
]
...
def main() -> int:
    ...
    for name in EXPECTED:
        ...
        if f'assets/va-claims-guided/{name}' not in page:
            errors.append(f'walkthrough does not reference {name}')

    cards = re.findall(r'<section class="card(?: active)?" data-card="(\d+)">(.*?)</section>', page, re.S)
    if len(cards) != 6:
        errors.append(f'expected 6 walkthrough cards, found {len(cards)}')
    for expected_number, (number, card) in enumerate(cards, 1):
        ...
        if card.count('type="checkbox"') != 1:
            errors.append(f'card {number}: requires one completion checkbox')
        if 'Mark step DONE' not in card:
            errors.append(f'card {number}: missing completion label')
        if '<figcaption>' not in card:
            errors.append(f'card {number}: missing accessible caption')

    for marker in ('Return to Instruction Page','Continue with help me complete this','vaClaimsStepStateV1','URLSearchParams'):
        if marker not in page:
            errors.append(f'missing focused-flow marker: {marker}')
    if 'SOURCE_GROUNDED_ASSISTANT' in page or 'Verified capability state' in page:
        errors.append('internal capability language exposed')

    assets = {name: hashlib.sha256((ASSET_DIR/name).read_bytes()).hexdigest() for name in EXPECTED if (ASSET_DIR/name).is_file()}
    body = {
        'schema_version':'3.0.0',
        'state':'PASS' if not errors else 'FAIL',
        'surface':PAGE.name,
        'design_contract':'PRIMARY_CHECKLIST_PLUS_FOCUSED_WALKTHROUGH',
        'shared_progress_key':'vaClaimsStepStateV1',
        'step_addressable':True,
