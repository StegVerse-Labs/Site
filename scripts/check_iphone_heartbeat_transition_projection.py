...
def main():
 ...
 h=HTML.read_text(); ht=visible_text(h); j=JS.read_text(); d=HANDOFF.read_text()
 for m in ('Historical HB29 → HB30 transition capsule','SHWP-IPHONE-HB30-TRANSITION-CAPSULE-001','HB29 / generation 29','HB30 / generation 30','TV/TVC','Generate historical HB30 receipt','HB32','every 10 ms','OSCILLATOR_ONLY','INDEPENDENT_HEARTBEAT_LIVE_PROOF_VERIFIED','not a prerequisite for current heartbeat continuity','HB-XXXXXXXX','HB32 = HB-0000000W'):
  if m not in ht:f.append('html visible text missing marker: '+m)
 if './heartbeat-transition.js' not in h:f.append('html missing script marker: ./heartbeat-transition.js')
 ...
 if f:
  [print('IPHONE_HB30_PROJECTION_FAIL:'+x) for x in f]; return 1
 print('IPHONE_HB30_PROJECTION_PASS historical=true current_hb32_continuous_10ms=true compact_id=HB-XXXXXXXX authority_effect=NONE credential_authority=TV/TVC physical_activation_required=false'); return 0
