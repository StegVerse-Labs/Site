...
def fetch(url: str) -> dict:
    request = Request(url, headers={"Accept": "application/json", "User-Agent": "StegVerse-Site-Activation-Evidence-Watch/2.0"})
...
def main() -> int:
    ...
    payload = {
        ...
        "generated_at": now,
        "state": "VERIFIED_RECEIPT_OBSERVED" if receipt_present and not blockers else "PENDING_RUNTIME_EVIDENCE",
        ...
        "next_machine_action": "continue_activation_import" if receipt_present and not blockers else "continue_runtime_activation_verification",
