...
def main() -> int:
    ...
    for key in ["auto_merge_enabled", "repo_mutation_from_monitor_enabled", "destructive_actions_allowed"]:
    ...
    if not ready_items:
        failures.append("ready_for_review is empty")
    if not not_ready_items:
        failures.append("not_ready_for_review is empty")

    seen: set[tuple[str, int]] = set()
    for section, expected in [("ready_for_review", READY), ("not_ready_for_review", NOT_READY)]:
