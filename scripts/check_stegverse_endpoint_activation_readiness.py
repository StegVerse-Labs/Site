...
def main() -> None:
    ...
    require(state.get("schema") == "stegverse.site.endpoint-activation-readiness.v2", "unexpected endpoint activation readiness schema")
    require(state.get("owner") == "issue/24", "endpoint activation owner must remain issue/24")
    require(state.get("state") == "SOVEREIGN_RUNTIME_AND_TVC_ROUTE_EVIDENCE_REQUIRED", "endpoint activation must remain blocked until sovereign runtime and TVC route evidence exists")
    ...
    require(legacy.get("result_sha256") == "27dbe3f846913e0cd66221e377f6e5adcbcb6e7842a3b4565b160c310063745d", "legacy activation observation hash mismatch")
