...
def main() -> int:
    ...
    for marker in test_markers:
        if marker not in test:
            return fail(f"browser test missing marker: {marker}")

    workflow_markers = [
        "StegMusic Browser Execution",
        "python -m playwright install --with-deps chromium",
        "python -m http.server 8000",
        "python tests/test_stegmusic_browser.py",
        "Upload browser execution receipt",
        "reports/stegmusic-browser-execution.json",
        "if: always()",
    ]
    ...
    print("STEGMUSIC_BROWSER_AUTOMATION_PASS")
