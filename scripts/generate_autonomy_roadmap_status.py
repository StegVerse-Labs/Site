...
def main() -> None:
    ...
    operational = int(summary.get("operationally_complete", 0) or 0)
    ...
    runtime_complete = operational > 0
    governance_complete = execution_complete and valid_evidence > 0
    continuity_complete = all((truth_complete, execution_complete, runtime_complete, governance_complete))
    ...
    phases = [
        ...
        },
        {
            ...
            "operation": "COMPLETE" if runtime_complete else "NOT_OBSERVED",
            "evidence": "SUFFICIENT" if runtime_complete else "MISSING",
            "exit_gate": "COMPLETE" if runtime_complete else "INCOMPLETE",
            "progress_percent": 100 if runtime_complete else 25,
            "completed": ["endpoint, freshness, machine-mode, and mobile-browser check specification"],
            "remaining": ([] if runtime_complete else ["execute endpoint checks", "execute browser user-flow tests", "record integration and deployment verification", "enforce evidence freshness"]),
            "blockers": [] if runtime_complete else ["runtime execution evidence has not been recorded"],
        },
        {
            "id": "governance-layer",
            "title": "Phase 4 — Governance layer",
            "implementation": "ADVANCED",
            "operation": "COMPLETE" if governance_complete else "PARTIAL",
        ...
        },
        {
            ...
            "operation": "COMPLETE" if continuity_complete else "PARTIAL",
