(() => {
  ...
  async function erc20Balance(address, block) {
    const result = await rpc('eth_call', [{ to: address, data: `0x70a08231${addressWord(C.wallet)}` }, block]);
  ...
  async function observeInventory() {
    ...
    const block = String(await rpc('eth_blockNumber', [])).toLowerCase();
    const nativeAtomic = BigInt(await rpc('eth_getBalance', [C.wallet, block]));
    const usdcAtomic = await erc20Balance(C.usdc, block);
    const wethAtomic = await erc20Balance(C.weth, block);
    if (usdcAtomic < C.sellAtomic) throw new Error(`insufficient USDC for exact validation entry: have ${usdcAtomic}, require ${C.sellAtomic}`);
    if (nativeAtomic <= 0n) throw new Error('native ETH gas reserve is empty');

    const assets = [
      { asset_id: `${C.chainId}:native:ETH`, symbol: 'ETH', asset_type: 'NATIVE', contract_address: null, amount_atomic: nativeAtomic.toString(), decimals: 18, state: 'OBSERVED', boundary: 'GAS_RESERVE', evidence_refs: [`base:block:${block}`] },
      { asset_id: `${C.chainId}:erc20:${C.usdc.toLowerCase()}`, symbol: 'USDC', asset_type: 'ERC20', contract_address: C.usdc, amount_atomic: usdcAtomic.toString(), decimals: 6, state: 'OBSERVED', boundary: 'IDLE_CAPITAL', evidence_refs: [`base:block:${block}`, 'scope:exact-validation-sell-asset'] },
      { asset_id: `${C.chainId}:erc20:${C.weth.toLowerCase()}`, symbol: 'WETH', asset_type: 'ERC20', contract_address: C.weth, amount_atomic: wethAtomic.toString(), decimals: 18, state: 'OBSERVED', boundary: wethAtomic > 0n ? 'ACTIVE_POSITION' : 'ACTIVE_POSITION_CAPACITY', evidence_refs: [`base:block:${block}`, 'scope:exact-validation-buy-asset'] },
    ];
    const basis = {
      wallet_address: C.wallet,
      chain_id: C.chainId,
  ...
  async function bindGas(candidate, quote, inventory) {
    ...
    const requiredGasAtomic = gas * gasPrice;
    const gasAsset = inventory.assets.find((asset) => asset.symbol === 'ETH' && asset.boundary === 'GAS_RESERVE');
    if (!gasAsset || BigInt(gasAsset.amount_atomic) < requiredGasAtomic) throw new Error(`insufficient native ETH gas reserve: have ${gasAsset?.amount_atomic || '0'}, require ${requiredGasAtomic}`);
    const quotedWeth = Number(BigInt(quote.amount_out)) / 1e18;
    if (!(quotedWeth > 0)) throw new Error('quote valuation invalid');
    const gasUsd = Number(requiredGasAtomic) / 1e18 * (12.5 / quotedWeth);
    if (!Number.isFinite(gasUsd) || gasUsd > C.maxGasUsd) throw new Error(`gas risk exceeds $${C.maxGasUsd}`);
    const bound = { ...candidate, gas: `0x${gas.toString(16)}`, gas_price: `0x${gasPrice.toString(16)}`, gas_required_atomic: requiredGasAtomic.toString(), gas_reserve_observed_atomic: gasAsset.amount_atomic, gas_reserve_sufficient: true, gas_estimate_usd: gasUsd.toFixed(8) };
  ...
  async function run() {
    ...
    try {
      ...
      candidate = await bindGas(candidate, quote, inventory);
      ...
      const receipt = {
        ...
        inventory_scope: inventory.inventory_scope, wallet_handoff_bundle_sha256: handoff.bundle_sha256, inventory_state_hash: inventory.inventory_state_hash, route_admission_sha256: admission.admission_sha256,
