...
The Site repository-wide COSV adoption work now has a zero-gap candidate under `SITE-COSV-REPOSITORY-WIDE-ADOPTION-001`. The validator derives the active denominator directly from the canonical Site claim loader, validates every live claim-source projection against its exact claim ID/state, and permits repository `VECTOR_PRESENT=true` only when the computed unindexed active task-ID set is empty. The current branch candidate is 48 effective active task IDs / 0 unindexed after evidence-backed retirement of the stale ERL KV propagation claim fragment; Site #396 and all other live owner semantics remain unchanged. This candidate is not final until exact-head hosted validation, merge, and post-merge loader recomputation confirm the same zero-gap state. See `docs/COSV_SITE_ADOPTION_MIRROR_HANDOFF.md`.
