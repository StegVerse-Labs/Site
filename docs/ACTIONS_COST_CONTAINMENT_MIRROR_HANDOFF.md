...
active_validation_claim: SITE-WORKFLOW-SURFACE-MINIMIZATION-268-B17R1-20260817
...
## Released baseline before B17R1
...
## Canonical claim-registry repair before B17R1

The first B17 exact-head run correctly exposed a pre-existing release-compaction defect: five released StegFin projection claims required by `scripts/check_stegfin_phone_projection.py` had been omitted from canonical `data/session-work-claims.json`. The provider-neutral implementation did not cause that failure. PR #342 was closed unmerged. Canonical main was repaired in `bdc8868d2a83e454fb9f2dd8d6a914c8f18f6ec7`, restoring the durable required projection claims instead of weakening the validator. B17R1 is reconstructed from that repaired current main.

## Active Batch 17R1 — Ecosystem Chat provider-neutral validation consolidation

```text
claim: SITE-WORKFLOW-SURFACE-MINIMIZATION-268-B17R1-20260817
branch: chore/site-ecosystem-chat-provider-neutral-validation-b17r1-20260817
...
- credential-clean canonical `.github/workflows/validate.yml` now explicitly runs `python3 scripts/check_ecosystem_chat_provider_neutral_binding.py` after canonical Site application validation;
- standalone `.github/workflows/check-ecosystem-chat-provider-neutral-binding.yml` is removed;
- canonical validation remains `permissions: {}`, rejects credential-bearing environment, anonymously fetches the exact source ref, uses preinstalled Python, performs no Actions checkout/setup/upload, no schedule, no repository writeback, no private LLM-adapter/StegCore runtime checkout, and no provider execution;
- no provider route, inference, HIL, StegOS, StegFin, wallet, deployment, publication, local-model, heartbeat, or product-activation authority is created.

Required exact-head release evidence:

1. Site Bootstrap Validate PASS;
2. `Validate Ecosystem Chat provider-neutral binding` PASS;
3. Site Handoff Orchestrator PASS;
4. Ecosystem Heartbeat Orchestration PASS;
5. Check StegFin Phone Projection PASS with all six durable projection claims present and USER_ONLY signing/broadcast intact;
6. `SESSION_WORK_CLAIMS_PASS`, canonical Site application PASS and ST-017 sandbox PASS;
7. workflow census `106 / canonical 3 / migration-required 103 / placeholders 0`;
...
Do not create a second scheduler, runtime, provider authority, review path, publication path, wallet authority, financial authority, or activation claim. Do not modify `check-hil-linkedin-launch-readiness.yml` while semantic drift remains review-required.
...
B17R1 provider-neutral validation consolidation: CLAIMED_FOR_INTEGRATION
...
Open B17R1 PR, validate exact head through all required lanes, inspect provider-neutral output and census, merge only on PASS/current-main compatibility, release B17R1 claim, update this handoff with immutable evidence, then inspect the next bounded unclaimed redundant/token-bearing workflow under Site #268.
...
active_B17R1: implemented / exact-head validation pending
session_consolidation: incomplete
```

## Archive condition

This session remains active while B17R1 is unreleased and additional Site workflow/token debt remains executable. Live HIL, sovereign runtime/inference, ordinary Healer execution and StegFin settlement remain separately worker-owned and are not inferred from source/CI state.
