...
active_implementation_claim: SITE-ST018-GITHUB-TOKEN-RETIREMENT-B17R1-20260817
active_validation_claim: SITE-ST018-GITHUB-TOKEN-RETIREMENT-B17R1-20260817
...
PR #333 HIL HTTPS receiver-probe regression workflow retired; regression suite folded into credential-clean HIL dispatcher
PR #337 Marketplace Coinbase hosted accessibility importer retired after terminal PAPER_ACCESSIBLE projection
```

## Active B17R1 — ST-018 token/evidence-transport retirement

```text
claim: SITE-ST018-GITHUB-TOKEN-RETIREMENT-B17R1-20260817
branch: chore/site-st018-validation-consolidation-b17r1-20260817
...
The removed `.github/workflows/capture-validation-evidence.yml` had `issues: write`, checkout/setup actions, two artifact-upload paths, `GH_TOKEN=${{ github.token }}`, and GitHub issue-comment mutation. It treated GitHub artifacts/comments as validation custody evidence even though current policy requires TV/TVC-only credentials and denies GitHub production/control-plane authority.

B17R1 preserves the ST-018 manifest/schema/source and consolidates deterministic execution into credential-clean canonical `.github/workflows/validate.yml`:
...
Canonical validation remains `permissions: {}`, refuses credential-bearing environment variables, anonymously fetches the exact ref, and uses preinstalled Python. No replacement scheduler, custody authority, project token, provider secret, or TV/TVC credential export is created.
...
Do not modify `check-hil-linkedin-launch-readiness.yml` while semantic drift remains `REVIEW_REQUIRED`. Cleanup may not mint provider, credential, runtime, publication, financial, wallet, model, HIL review or Master Records authority.
...
Validate B17R1 exact head. If green, merge, recensus current main, reconcile Site #141, release B17R1, finalize this handoff, then inspect the next bounded unclaimed token-bearing/redundant workflow under Site #268.
