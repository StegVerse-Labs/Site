...
`data/transition-discovery-status-v1.json` records the machine-readable status handoff and active blocker surface.
...
Machine-readable status: data/transition-discovery-status-v1.json
...
machine status schema is stegverse.transition_discovery_status.v1
...
T13 and T14 remain receipt-backed in canonical, JSON, contract, receipt, and status state
...
machine status identifies Publisher closure evidence as the only mirror-activation blocker
...
The workflow runs on manual dispatch and on pushes that affect the canonical discovery state, machine-readable state, machine-readable page contract, machine-readable receipt, machine-readable status, human-readable receipt, status handoff, renderer, seven root-level public pages, this document, or the checker.
