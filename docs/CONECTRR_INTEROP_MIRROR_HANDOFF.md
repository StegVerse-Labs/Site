...
Status: PAGE_RUNTIME_BINDING_BROWSER_CORRELATION_SOURCE_PRESERVATION_AND_RECONSTRUCTION_IMPLEMENTED; LIVE_CONECTRR_OUTPUT_PENDING
```
...
`assets/ecosystem-node-views.js` exposes `importCanonicalEvents(events)` and now loads `assets/conectrr-interop.js` only after the canonical event API exists. Imported events are cloned before admission, duplicate identifiers are rejected, unresolved parent references are rejected, and admitted records are frozen.

`assets/conectrr-interop.js` loads the independent-evaluation fixture, imports the Conectrr evidence event and StegVerse decision event together, requires both governed records to render, and executes bidirectional selection checks using stable event identifiers. It publishes `data-conectrr-browser-test="pass"` only after both directions succeed and fails closed otherwise.

`check_conectrr_runtime_projection.py` is bound into canonical Site application validation and invokes `check_conectrr_browser_projection.py`, so runtime loading, rendering, and bidirectional correlation are included in the aggregate gate.
...
```text
Add deployed-preview observation for data-conectrr-interop=loaded
Add deployed-preview observation for data-conectrr-browser-test=pass
Add explicit export replay verification against the browser-loaded stream
Replace fixture source_bytes_hash marker with the exact real-source digest when live output exists
Add live-output adapter conformance fixture without semantic normalization
```
...
browser PASS != live external interoperability
