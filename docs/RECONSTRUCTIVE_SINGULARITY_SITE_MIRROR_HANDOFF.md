# Reconstructive Singularity Site Mirror Handoff

## Status

`SUPERSEDED_BY_RECONSTRUCTABLE_SINGULARITY`

This handoff is retained for provenance only. **The Reconstructive Singularity** is no longer a separate current Site paper and is not indexed from `Papers.html`.

## Superseding publication

Canonical current formalism:
```text
https://admissibility.stegverse.org/formalisms/reconstructable-singularity
```

Current Site handoff:
```text
docs/RECONSTRUCTABLE_SINGULARITY_SITE_MIRROR_HANDOFF.md
```

The legacy route `/reconstructive-singularity.html` is retained only as a compatibility redirect to the canonical Admissibility Wiki formalism.

## Historical source

The superseded paper was originally bound to:
```text
Admissible-Existence/AE/docs/papers/reconstructive-singularity-scholarly-review.md
source creation commit: 20a09b996759044d6034fd896c90c0ae85b0270b
Site page commit: 185d39c30a5cdc2368bc9aad149fca8704afa90c
Site index commit: 338b62604d99576758b491274e7dba97028c51de
```

Historical validation claims are preserved as historical provenance only and are not automatically inherited as validation of the newer Reconstructable Singularity formalism.
