...
This file records the active correction to the public Humans as the Interoperability Layer experiment path.
...
The client now discovers a same-origin conforming gateway first and retains the historical external gateway only as a compatibility candidate. Failure of one candidate no longer prevents discovery of another conforming endpoint.
...
The public URL, canonical PDF bytes, Site metadata, prompt text and hash, gateway readiness contract, and first operator receipt must all agree before the experiment is declared active.
